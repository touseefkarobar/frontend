...
const StatTile = ({ label, value, accent }) => (
  <div className="rounded-3xl bg-slate-900/60 p-5 shadow-[0_12px_40px_-20px_rgba(59,130,246,0.6)] ring-1 ring-white/10 backdrop-blur">
    ...
    <p className={`mt-3 text-2xl font-semibold tracking-tight sm:text-3xl ${accent}`}>{value}</p>
  </div>
);
...
const formatCurrencyValue = (value, currencyCode) => {
  const safeCurrency = currencyCode && typeof currencyCode === 'string' ? currencyCode.toUpperCase() : DEFAULT_CURRENCY;
  try {
...
function App() {
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const normalized = (salaryCurrency || '').trim().slice(0, 3).toUpperCase() || DEFAULT_CURRENCY;
  ...
  const normalizedCurrency = (salaryCurrency || '').trim().slice(0, 3).toUpperCase() || DEFAULT_CURRENCY;
  const parsedHourlyRate = parseFloat(hourlyRate) || 0;
  const calculatedSalary = parsedLoggedHours * parsedHourlyRate;
  const formattedSalary = formatCurrencyValue(calculatedSalary, normalizedCurrency);
  const formattedHourlyRate = parsedHourlyRate ? formatCurrencyValue(parsedHourlyRate, normalizedCurrency) : null;
  ...
  const workingDaysRemaining = Math.max(totalWorkingDays - workingDaysToDate, 0);
          ...
          <div className="w-full max-w-md space-y-8 rounded-[32px] border border-white/10 bg-slate-900/70 p-8 shadow-[0_30px_80px_-40px_rgba(56,189,248,0.7)] backdrop-blur">
            <div className="text-center">
              ...
              <p className="mt-2 text-sm text-slate-300">
                Securely connect your TeamLogger account to unlock personalised insights.
              </p>
            </div>
    ...
    <div className="relative min-h-screen overflow-hidden bg-slate-950 text-slate-100">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-5xl flex-col pb-32">
        <header className="relative overflow-hidden rounded-b-[40px] bg-gradient-to-br from-indigo-600 via-purple-600 to-sky-500 px-6 pb-28 pt-16 text-white shadow-[0_45px_120px_-50px_rgba(56,189,248,0.65)]">
          ...
          <div className="relative z-10 flex flex-col gap-10">
            <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.35em] text-white/70">Hello</p>
                <h1 className="mt-3 text-3xl font-semibold sm:text-4xl">
                  {accountName ? `${accountName.split(' ')[0]}'s workspace` : 'Your workday companion'}
                </h1>
                {companyName ? <p className="mt-2 text-sm text-white/80">{companyName}</p> : null}
              </div>
              <div className="flex items-center gap-3 self-start rounded-full bg-white/15 px-5 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-white/80 backdrop-blur">
            ...
            <div className="rounded-3xl border border-white/15 bg-white/15 p-6 backdrop-blur-xl sm:p-8">
              <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.4em] text-white/70">Calculated salary</p>
                  <p className="mt-4 text-4xl font-semibold sm:text-5xl">{showSalary ? formattedSalary : '••••••'}</p>
                  <p className="mt-3 text-sm text-white/80">
                    {parsedHourlyRate > 0
                      ? `Based on ${formatNumber(parsedLoggedHours)} logged hours at ${formattedHourlyRate} / hr.`
                      : 'Set your hourly rate in settings to calculate earnings automatically.'}
                  </p>
                </div>
                <div className="flex flex-col items-start gap-3 sm:items-end">
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-full bg-white/25 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-black/20 transition hover:bg-white/35"
                  >
                    {showSalary ? 'Hide amount' : 'Show amount'}
                  </button>
                ...
                <button
                  ...
                  className={`flex items-center gap-3 rounded-2xl px-5 py-3 text-sm font-semibold transition ${
                    activeSection === tab.key
                      ? 'bg-white/20 text-white shadow-[0_18px_40px_-30px_rgba(255,255,255,0.9)]'
        ...
        <main className="-mt-16 flex-1 space-y-12 px-6 pt-0">
          <section className={`${activeSection === 'overview' ? 'block' : 'hidden md:block'} space-y-8`}>
            <div className="grid gap-4 lg:grid-cols-3">
              <div className="rounded-3xl bg-slate-900/80 p-6 shadow-[0_24px_80px_-40px_rgba(99,102,241,0.7)] ring-1 ring-white/10 backdrop-blur sm:p-8">
                <p className="text-xs font-semibold uppercase tracking-[0.35em] text-slate-400">{hoursStatusLabel}</p>
                ...
                <p className="mt-3 text-sm text-slate-300">
                  You are {hourDelta >= 0 ? 'ahead of' : 'behind'} the expected pace of {formattedExpectedHours} hours.
                </p>
              </div>
              ...
              <div className="rounded-3xl bg-slate-900/80 p-6 shadow-[0_24px_80px_-40px_rgba(14,165,233,0.65)] ring-1 ring-white/10 backdrop-blur sm:p-8">
                <p className="text-xs font-semibold uppercase tracking-[0.35em] text-slate-400">Logged vs target</p>
                <p className="mt-4 text-3xl font-semibold text-white sm:text-4xl">
                  {formattedLoggedHours} <span className="text-lg text-slate-400">hours</span>
                </p>
                <p className="mt-3 text-sm text-slate-300">Monthly target: {formatNumber(totalTargetHours)} h</p>
                <p className="mt-1 text-sm text-slate-300">Expected by today: {formattedExpectedHours} h</p>
              </div>
              ...
              <div className="rounded-3xl bg-slate-900/80 p-6 shadow-[0_24px_80px_-40px_rgba(16,185,129,0.6)] ring-1 ring-white/10 backdrop-blur sm:p-8">
                <p className="text-xs font-semibold uppercase tracking-[0.35em] text-slate-400">Rhythm outlook</p>
                <p className="mt-4 text-3xl font-semibold text-white sm:text-4xl">
                  {formatNumber(workingDaysRemaining)} <span className="text-lg text-slate-400">days left</span>
                </p>
                <p className="mt-3 text-sm text-slate-300">
                  Remaining hours to target: {formatNumber(Math.max(hoursToTarget, 0))} h
                </p>
                <p className="mt-1 text-sm text-slate-400">
                  Total working days this month: {formatNumber(totalWorkingDays)}
                </p>
              </div>
            </div>

            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <StatTile label="Total working days" value={formatNumber(totalWorkingDays)} accent="text-indigo-200" />
            ...
            <div className="grid gap-4 lg:grid-cols-[1.2fr,0.8fr]">
              <div className="space-y-6 rounded-3xl bg-slate-900/70 p-6 ring-1 ring-white/10 backdrop-blur sm:p-8">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <h2 className="text-lg font-semibold text-white">TeamLogger pulse</h2>
                  <span className="rounded-full bg-slate-800/80 px-4 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-slate-300">
                    {lastSyncLabel}
                  </span>
                </div>
                <p className="text-sm text-slate-300">
                  Syncs keep your overview fresh. Key metrics from your latest report are highlighted below.
                </p>
                <dl className="grid gap-4 sm:grid-cols-2">
                  <div className="rounded-2xl bg-slate-950/70 p-4 ring-1 ring-white/5">
                    <dt className="text-xs font-semibold uppercase tracking-[0.35em] text-slate-500">Tracked duration</dt>
                    <dd className="mt-3 text-xl font-semibold text-white">
                      {teamLoggerTotals ? formatDuration(teamLoggerTotals.totalWorkedMilliseconds) : '—'}
                    </dd>
                    <dd className="mt-1 text-xs text-slate-400">
                      ≈ {teamLoggerTotals ? formatNumber(teamLoggerTotals.totalWorkedHours) : '—'} hours total
                    </dd>
                  </div>
                  <div className="rounded-2xl bg-slate-950/70 p-4 ring-1 ring-white/5">
                    <dt className="text-xs font-semibold uppercase tracking-[0.35em] text-slate-500">Active minutes</dt>
                    <dd className="mt-3 text-xl font-semibold text-white">
                      {teamLoggerStats ? formatPercentageValue(teamLoggerStats.activeMinutesRatio) : '—'}
                    </dd>
                    <dd className="mt-1 text-xs text-slate-400">Share of focused time</dd>
                  </div>
                </dl>
                <p className="rounded-2xl border border-white/5 bg-white/5 px-5 py-4 text-sm text-slate-200">
                  Keep holidays updated to maintain precise projections and maintain your lead.
                </p>
              </div>

              <div className="rounded-3xl bg-gradient-to-br from-slate-900/80 via-slate-900/70 to-slate-950/80 p-6 ring-1 ring-white/10 backdrop-blur sm:p-8">
                <h3 className="text-lg font-semibold text-white">Quick actions</h3>
                <ul className="mt-4 space-y-3 text-sm text-slate-300">
                  <li className="flex items-center gap-3 rounded-2xl bg-slate-950/60 px-4 py-3 ring-1 ring-white/5">
                    <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-indigo-500/30 text-indigo-100">1</span>
                    Update weekend days in settings to tune working day calculations.
                  </li>
                  <li className="flex items-center gap-3 rounded-2xl bg-slate-950/60 px-4 py-3 ring-1 ring-white/5">
                    <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-sky-500/30 text-sky-100">2</span>
                    Add upcoming holidays to forecast remaining workload accurately.
                  </li>
                  <li className="flex items-center gap-3 rounded-2xl bg-slate-950/60 px-4 py-3 ring-1 ring-white/5">
                    <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-emerald-500/30 text-emerald-100">3</span>
                    Refresh TeamLogger sync whenever you log new sessions.
                  </li>
                </ul>
              </div>
            </div>
          ...
          <section className={`${activeSection === 'settings' ? 'block' : 'hidden md:block'} space-y-8`}>
            <div className="grid gap-6 lg:grid-cols-[1.15fr,0.85fr]">
              <div className="space-y-6 rounded-3xl bg-slate-900/70 p-6 ring-1 ring-white/10 backdrop-blur sm:p-8">
                <div>
                  <h2 className="text-lg font-semibold text-white">Calendar configuration</h2>
                  <p className="mt-2 text-sm text-slate-300">
                    Tailor how your working month is calculated by selecting weekends, daily targets, and holidays.
                  </p>
                </div>

                <div className="space-y-5">
                  <div>
                    <h3 className="text-xs font-semibold uppercase tracking-[0.35em] text-slate-400">Weekend days</h3>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <label className="flex flex-col gap-2">
                      <span className="text-sm font-medium text-slate-300">Target hours per working day</span>
                      <input
                        type="number"
                        min="0"
                        step="0.25"
                    ...
                    <label className="flex flex-col gap-2">
                      <span className="text-sm font-medium text-slate-300">Add holiday</span>
              ...
              <div className="space-y-6">
                <div className="rounded-3xl bg-slate-900/70 p-6 ring-1 ring-white/10 backdrop-blur sm:p-8">
                  <h3 className="text-lg font-semibold text-white">Earnings preferences</h3>
                  <p className="mt-2 text-sm text-slate-300">
                    Configure your hourly rate and currency to unlock salary projections on the home view.
                  </p>
                  <div className="mt-5 grid gap-4 sm:grid-cols-[2fr,1fr]">
                    <label className="flex flex-col gap-2">
                      <span className="text-sm font-medium text-slate-300">Hourly rate</span>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                ...
                <div className="space-y-4 rounded-3xl bg-slate-900/70 p-6 ring-1 ring-white/10 backdrop-blur sm:p-8">
                  <div>
                    <h3 className="text-lg font-semibold text-white">Real-time effort tracking</h3>
                    <p className="mt-2 text-sm text-slate-300">
                      We automatically sync your on-computer hours. Adjust below to model different outcomes.
                    </p>
                  </div>

                  <div className="flex flex-col gap-2">
                    <span className="text-sm font-medium text-slate-300">On-computer hours this month</span>
                    <input
                      type="number"
                      min="0"
                      step="0.25"
                  ...
                  <div className="rounded-2xl bg-indigo-500/10 px-5 py-4 text-sm text-indigo-100">
                    Keep your working rhythm consistent to stay aligned with targets and salary goals.
                  </div>
                </div>
              </div>
            </div>
          ...
          <section className={`${activeSection === 'insights' ? 'block' : 'hidden md:block'} space-y-8`}>
            <div className="grid gap-6 lg:grid-cols-[1.15fr,0.85fr]">
              <div className="space-y-6 rounded-3xl bg-slate-900/70 p-6 ring-1 ring-white/10 backdrop-blur sm:p-8">
                <div>
                  <h2 className="text-lg font-semibold text-white">TeamLogger sync</h2>
                  <p className="mt-2 text-sm text-slate-300">
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl bg-slate-950/70 p-6 ring-1 ring-white/5">
                    <p className="text-xs font-semibold uppercase tracking-[0.35em] text-slate-400">Connected account</p>
                    ...
                    <dl className="mt-4 space-y-3 text-sm text-slate-200">
                      <div className="flex flex-wrap justify-between gap-3 text-slate-300">
                        <dt>Email</dt>
                        <dd className="font-medium text-white">{auth.account?.email ?? '—'}</dd>
                      </div>
                      <div className="flex flex-wrap justify-between gap-3 text-slate-300">
                        <dt>Company</dt>
                        <dd className="font-medium text-white">{companyName || '—'}</dd>
                      </div>
                      <div className="flex flex-wrap justify-between gap-3 text-slate-300">
                        <dt>Username</dt>
                        <dd className="font-medium text-white">{auth.account?.username ?? '—'}</dd>
                      </div>
                      <div className="flex flex-wrap justify-between gap-3 text-slate-300">
                        <dt>Last synced</dt>
                        <dd className="font-medium text-white">{lastSyncLabel}</dd>
                      </div>
                    </dl>
                  </div>
              ...
              <div className="space-y-4 rounded-3xl bg-slate-900/70 p-6 ring-1 ring-white/10 backdrop-blur sm:p-8">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-300">
                    Results are parsed automatically from matching <code>totalWorked*</code> or tracked duration fields.
                  </p>
                </div>
                ...
                {teamLoggerTotals ? (
                  <div className="space-y-5">
                    <div className="rounded-2xl bg-slate-950/70 p-6 ring-1 ring-white/5">
                      <p className="text-xs font-semibold uppercase tracking-[0.35em] text-slate-400">Tracked duration</p>
                      <p className="mt-3 text-3xl font-semibold text-white">
                          ...
                          <div className="rounded-3xl bg-slate-950/70 p-6 ring-1 ring-white/5">
                            ...
                            <dl className="mt-4 space-y-3 text-sm text-slate-200">
                              <div className="flex justify-between gap-3 text-slate-300">
                                <dt>Timer status</dt>
                                <dd className="font-medium text-white">{teamLoggerStats.las.tStatus ?? '—'}</dd>
                              </div>
                              <div className="flex justify-between gap-3 text-slate-300">
                                <dt>User status</dt>
                                <dd className="font-medium text-white">{teamLoggerStats.las.uStatus ?? '—'}</dd>
                              </div>
                              <div className="flex justify-between gap-3 text-slate-300">
                                <dt>Idle time</dt>
                                <dd className="font-medium text-white">{formatSecondsValue(teamLoggerStats.las.idleSecs)}</dd>
                              </div>
                              <div className="flex justify-between gap-3 text-slate-300">
                                <dt>App version</dt>
                                <dd className="font-medium text-white">{teamLoggerStats.las.cVersion ?? '—'}</dd>
                              </div>
                              <div className="flex justify-between gap-3 text-slate-300">
                                <dt>Last sync</dt>
                                <dd className="font-medium text-white">{formatTimestamp(teamLoggerStats.las.ts)}</dd>
                              </div>
                            </dl>
                          </div>
                        ) : null}
                    ...
                    <details className="rounded-2xl bg-slate-950/60 p-4 text-sm text-slate-200 ring-1 ring-white/5">
                      <summary className="cursor-pointer font-medium text-white">View raw response</summary>
        ...
        <footer className="px-6 pb-8 pt-6">
          <div className="rounded-3xl bg-slate-900/70 px-6 py-5 text-center text-xs text-slate-400 ring-1 ring-white/10 backdrop-blur">
            Crafted with React & Tailwind · Stay balanced and inspired ✨
          </div>
        </footer>
        ...
        <nav className="fixed bottom-6 left-1/2 z-30 w-[min(420px,calc(100%-32px))] -translate-x-1/2">
          <div className="flex items-center gap-2 rounded-3xl bg-slate-900/90 p-2 shadow-[0_24px_60px_-30px_rgba(15,23,42,0.8)] backdrop-blur">
            {SECTION_TABS.map((tab) => (
              ...
              >
                <tab.icon className={`h-5 w-5 ${activeSection === tab.key ? 'text-sky-200' : 'text-slate-500'}`} />
                {tab.label}
              </button>
            ))}
          </div>
